...
cluster_plot_52map <- function(data_in){
  ...
  stannote <- as.data.table(aggregate(cbind(x, y) ~ state, data = df, FUN=function(x)mean(range(x))))
  snames <- data.table(state = state.name, abb = state.abb) # these are dataset within R
  setkey(stannote, state)
  setkey(snames, state)
  stannote <- snames[stannote]
  
  x_boundary <- 1850384.826
  x_limits <- c(-5000, NA)
  
  # map_dat <- merge(df,data_in,by='state')
  p <- ggplot(data = df,mapping=aes(x=x,y=y,group=group,fill=values))
  p + geom_polygon(color = "gray90", size = 0.1) + geom_text(data=stannote, aes(x=x,y=y, label=state), size=3, inherit.aes=F)
}
